...
class GoogleAdsReportController extends Controller
{
    ...
    {
        if (Auth::user('admin')->can('google-ad-update') != true) {
    ...
    {
        if (Auth::user('admin')->can('google-ad-read') != true) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->filter) {
            $selectedAd = GoogleAd::find($request->filter);
        }

        $get_google_report = GoogleReport::where('client_id', auth('web')->user()->id)->first();
        ...
        if (!$get_google_report || isset($selectedAd)) {
            ...
            $acct_id = auth('web')->user()->customer_id;
        ...
        // $tour = Tour::firstOrCreate(['code' => 'GOOGLE_REPORT'], ['name' => 'Google Report']);
        // $client_tour = ClientTour::where(['client_id' => auth('web')->user()->id, 'tour_id' => $tour->id])->first();

        $googleAds = GoogleAd::where('client_id', auth('web')->user()->id)->get();
    ...
    public function save_google_report(Request $request)
    {
        ...
        if (
            Auth::user('admin')->can('google-ad-write') != true ||
            Auth::user('admin')->can('google-ad-update') != true
        ) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function download_pdf()
    {
        ...
        if ($get_google_report) {
            $get_add_account_name = GoogleAdsAccount::where('account_id', $get_google_report->act_id)->first();
        }
    ...
    private function get_google_campaigns($start_date, $end_date, $acct_id, $access_token, $devloper_token, $filter = null)
    {
        $curl = curl_init();
        ...
        if (isset($decoded_response['results'])) {
            $googleAdsResoucesNames = GoogleAd::where('client_id', auth('web')->user()->id)->pluck('campaign_resource_name')->toArray();
            if ($filter) {
    ...
    private function get_google_ads_group($start_date, $end_date, $acct_id, $access_token, $devloper_token, $filter = null)
    {
        $curl = curl_init();
    ...
    private function get_google_keywords($start_date, $end_date, $acct_id, $access_token, $devloper_token, $filter = null)
    {
        $curl = curl_init();
    ...
    private function get_google_ads($start_date, $end_date, $acct_id, $access_token, $devloper_token, $filter = null)
    {
        $curl = curl_init();
    ...
    private function get_performance_devices($start_date, $end_date, $acct_id, $access_token, $devloper_token, $filter = null)
    {
        $curl = curl_init();
    ...
    private function get_summary_graph_data($start_date, $end_date, $acct_id, $access_token, $devloper_token, $filter = null)
    {
        $curl = curl_init();
    ...
    private function get_performance_data_for_graph($start_date, $end_date, $acct_id, $access_token, $devloper_token, $filter = null)
    {
        $curl = curl_init();
    ...
    private function save_customer_account_detail()
    {
        ...
        $customer_ids = $this->get_accessible_customer($devloper_token, $access_token);
        
        $customer_ids_array = explode(', ', $customer_ids);

        foreach ($customer_ids_array as $customer_id) {
            $customer_detail = $this->get_customer_details_by_id($customer_id, $access_token, $devloper_token);
            if (isset($customer_detail['results']) && !empty($customer_detail['results'])) {
                $save_customer = new GoogleAdsAccount;
                $save_customer->account_title = $customer_detail['results'][0]['customer']['descriptiveName'] ?? '';
                $save_customer->account_id = $customer_detail['results'][0]['customer']['id'];
                $save_customer->save();
            }
        }
    }
