    ...
    <div class="ms-auto">
        <div class="btn-group">
            ...
            <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end">
                ...
                <a class="dropdown-item text-danger" href="javascript:void(0);" onclick="ajaxRequest(this)" data-url="{{route('admin.lead-management.delete',$data->hashid)}}">Delete Client</a>
            </div>
        </div>
    </div>
</div>
...
<div class="d-flex align-items-center">
    <form action="{{ route('admin.lead-management.set_follow_up') }}" method="POST" id="followUpForm">
        @csrf
        <div class="mb-3">
            <input type="hidden" name="id" value="{{ $data->id }}">
            @if(!empty($data->follow_up_date_time))
                <a href="javascript:;" onclick="ajaxRequest(this)" data-url="{{route('admin.lead-management.unset_follow_up',$data->id)}}" class="text-danger" style="float: right;position: relative;top: 30px;left: 20px;"><i class="fa fa-trash"></i></a>
            @endif
            ...
            <!-- <input class="result form-control" type="text" name="follow_up_date_time" id="date-time" value="{{ $data->follow_up_date_time }}" placeholder="No Follow Up Scheduled"> -->
                                            ...
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="fs-5 ms-auto dropdown">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        @if (isset($item->message_template_id) && !empty($item->message_template_id))
                                                            <li><a class="dropdown-item text-success float-end" href="{{ route('admin.message-template.temp_details', hashids_encode($item->message_template_id)) }}">Go To Message Detail</a></li>
                                                            <li><a class="dropdown-item text-danger float-end" onclick="ajaxRequest(this)" data-url="{{route('admin.lead-management.activity_delete',$item->id)}}" href="Javascript:void(0);">Delete</a></li>
                                                        @elseif(isset($item->email_template_id) && !empty($item->email_template_id))
                                                            <li><a class="dropdown-item text-success float-end" href="{{ route('admin.email-template.temp_details', hashids_encode($item->email_template_id)) }}">Go To Email Detail</a></li>
                                                            <li><a class="dropdown-item text-danger float-end" onclick="ajaxRequest(this)" data-url="{{route('admin.lead-management.activity_delete',$item->id)}}" href="Javascript:void(0);">Delete</a></li>
                                                        @elseif($item->type == "file")
                                                        @php
                                                            $file_id = hashids_encode($item->file_id)
                                                        @endphp
                                                            <li><a class="dropdown-item text-success" href="{{ route('admin.file_manager.file_detail', $file_id) }}">Go To File Detail</a></li>
                                                            <li><a class="dropdown-item text-danger float-end" onclick="ajaxRequest(this)" data-url="{{route('admin.lead-management.activity_delete',$item->id)}}" href="Javascript:void(0);">Delete</a></li>
                                                        @else
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('admin.lead-management.save') }}" method="post" id="leadForm">
                ...
                <div class="modal-body">
                    ...
                    <textarea name="note" class="form-control" id="client_note" cols="30" rows="10" required></textarea>
                </div>
...
<div class="modal fade" id="activityModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.lead-management.activity_save') }}" method="post" id="activityForm">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="row" id="attachment_div">
                        <!-- Add Attachment Card -->
                        <div class="card radius-10 border shadow-none mb-3" style="width: 467px; cursor: pointer; margin-left: 27px;" id="upload-card">
                        ...
                        <div id="file-preview-container" style=" padding-left: 27px; "></div>

                        <!-- Optional max file count message -->
                        <div id="file-count" style="color: red; display: none; padding-left: 27px; "></div>
                    </div>
...
<div class="modal fade" id="changeStatusModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.lead-management.update_status') }}" method="post" id="UpdateStatusForm">
                @csrf
                <div class="modal-body">
                    <input type="hidden" name="id" value="{{ $data->id }}">
                    <div class="form-group">
                        <select name="status" class="form-control" id="leadstatus" required>
                            ...
                            @foreach (['unmarked' => 'Unmarked', 'spam' => 'Spam','junk' => 'Junk', 'uncontacted' => 'Uncontacted', 'contacted' => 'Contacted'] as $k => $val)
...
<script>
    ...
    // $('#activity-date-time').bootstrapMaterialDatePicker({
    //     format: 'YYYY-MM-DD hh:mm',
    ...
    $(document).on('change', '.add_group_lead', function () {
        ...
        $.ajax({
            url: "{{ route('admin.lead-management.group_lead_save') }}",
            type: 'POST',
            data: data,
