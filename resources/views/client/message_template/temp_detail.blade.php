...
@section('content')
<div class="row">
    <input type="hidden" value="{{ @$send_message}}" id="openmodal">
    <div class="col">
    ...
    <div class="ms-auto">
        ...
        <div class="btn-group">
            ...
            <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end">
                ...
                <a class="dropdown-item edit-temp" href="javascript:void(0);" data-data="{{ $data }}">Edit message template</a>
                <a class="dropdown-item copy-template" href="javascript:void(0);" data-data="{{ $data }}">Create copy of message</a>
                @if ($data->message_activity()->count() == 0)
                <a class="dropdown-item text-danger" href="javascript:void(0);" onclick="ajaxRequest(this)" data-url="{{route('user.message-template.delete',$data->hashid)}}">Delete message template</a>
                @endif
            </div>
        </div>
    </div>
...
<div class="row mt-4">
    <div class="col-12 col-md-6">
        ...
        <div class="card">
            <div class="card-body msg_preview" style=" height: 201px;">
                <div class="row g-0">
                    <div class="col-12">
                                ...
                                <div class="col-12 col-lg-12 d-flex">
                                    <div class="card shadow radius-10 w-100">
                                        <div class="card-body">
                                            ...
                                            <small class="text-muted">Message Template Created</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<div class="modal fade" id="templateNoteModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('user.message-template.save') }}" method="post" id="templateNoteForm">
                @csrf
...
<script>
    ...
    $(document).ready(function (){
        ...
        if (openSendModal !== null && openSendModal === 'send_message') {
            $('.send-template').click();
        }
    });
                ...
                let div = `<div id="file-preview-container-${clientData.lead_id}">
                            <div class="card radius-10 border shadow-none mb-3 file-preview" style="width: 467px;">
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        <div class="">
                                            <p class="mb-1">${clientData.name} (${clientData.num})</p>
                                        </div>
                                        <div class="ms-auto">
                                            <button type="button" class="btn btn-success send_msg" id="snd_btn${clientData.lead_id}" data-id="${clientData.lead_id}" data-num="${clientData.num}" data-name="${clientData.name}">SEND</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    $('body').on('click', '.send_msg', function() {
        ...
        $.ajax({
            url: '{{ route("user.message-template.send") }}', // URL from your Laravel route
            type: 'POST', // Method
            data: {
            ...
            success: function(response) {
                let whatsappLink = "https://wa.me/65" + num + "?text=" + encodeURIComponent(tempMessage);
    ...
    $(".copy-template").click(function () {
        ...
        var url = "{{ route('user.message-template.copy_temp') }}";
