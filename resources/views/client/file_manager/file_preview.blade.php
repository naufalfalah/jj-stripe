...
<body>
    ...
    <div class="wrapper">
        ...
        <main class="page-content">
            <h3 class="fw-bold mt-3">{{ $data->file_name }}</h3>
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        @if(in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif']))
                            <!-- Display image -->
                            <img src="{{ asset($data->file_path) }}" width="50%" height="500" alt="Image">
                        @elseif($fileExtension == 'pdf')
                            <!-- Display PDF -->
                            <iframe src="{{ asset($data->file_path) }}" width="100%" height="500"></iframe>
                        @elseif(in_array($fileExtension, ['doc', 'docx', 'xlsx', 'ppt', 'pptx']))
                            <!-- Provide a download link for document files -->
                            <a href="{{ asset($data->file_path) }}" target="_blank">Download Document</a>
                        @else
                            <!-- For other files, just provide a download link -->
                            <a href="{{ asset($data->file_path) }}" target="_blank">Download File</a>
                        @endif
                    </div>
                </div>
        ...
        <footer class="footer">
            <div class="footer-text">
                ...
                <img src="{{asset(check_file($data->user->image,'user'))}}" class="client-img mt-2" alt="">
                <h4 class="text-dark mt-3">{{ $data->user->client_name }}</h4>
                <p class="text-dark">{{ $data->user->phone_number }}</p>
                <p class="text-dark">{{  $data->user->user_agency->name}}</p>

                <a type="button" href="tel:{{ $data->user->phone_number}}" class="btn btn-info px-5"><i class="fa-solid fa-phone-flip"></i> Call</a>
                <a type="button" href="sms://{{ $data->user->phone_number}}" class="btn btn-info px-5"><i class="fa-solid fa-comment-sms"></i> SMS</a>
                <a type="button" href="https://wa.me/{{ $data->user->phone_number }}" target="_blank" class="btn btn-info px-5"><i class="fa-brands fa-whatsapp"></i> WhatsApp</a>
            </div>
        </footer>
        <!--end footer-->
