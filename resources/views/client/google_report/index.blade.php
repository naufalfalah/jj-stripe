...
@push('styles')
    ...
    <link rel="stylesheet" href="{{ asset('front') }}/assets/plugins/datatable/css/dataTables.bootstrap5.min.css" />
    ...
    <div class="form-group my-2">
        ...
        <select id="google_ad_filter" class="form-select">
            ...
            @foreach ($googleAds as $googleAd)
                <option value="{{ $googleAd->id }}">{{ $googleAd->ad_name }}</option>
            @endforeach
        </select>
    </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-xl-8" id="step-2">
                ...
                <a href="javascript:void(0);" id="refresh_google_report" class="btn btn-primary float-end me-1 mb-2"
                    data-bs-toggle="tooltip" data-bs-placement="top"  title="Refresh Google googleAds Data">
...
@section('page-scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#google_ad_filter').on('change', function() {
                let googleAdFilter = $('#google_ad_filter').val();
                window.location.href = "{{ route('user.google-ads-report.google_ads_report') }}" + "?filter=" + googleAdFilter;
            });
        ...
        $(document).on('change', '#expiry_date', function () {
            ...
            update_expiry_date("{{ route('user.google-ads-report.update_act_expiry_date') }}",data,'POST');
        });
        ...
        $('#generate_pdf_btn').click(function(event) {
            ...
            if (expiry_date && (expiry_date === today || expiry_date < today)) {
                ...
                window.open("{{ route('user.google-ads-report.download_pdf') }}", '_blank') 
        ...
        $(document).ready(function() {
            if (tour && !clientTour) {
                ...
                if (step1) {
                    steps.push({ 
                        element: step1,
                        intro: "View and download invoices with details like Invoice ID, Date, and Total Amount."
                    });
                }
                ...
                if (step2) {
                    steps.push({
                        element: step2,
                        intro: "Use these buttons to set a date range, refresh data, or generate a PDF report."
                    });
                }
                ...
                if (step3) {
                    steps.push({ 
                        element: step3,
                        intro: "Select the account expiry date for managing report periods."
                    });
                }
                ...
                if (step4) {
                    steps.push({
                        element: step4,
                        intro: "Explore the summary, performance, and campaign data in the tables and charts."
                    });
                }
                ...
                if (steps.length > 0) {
                    steps.push({
                        intro: "This concludes the tour of the Google Report page. You're now ready to efficiently manage and analyze your reports."
                    });
                }
